...
async function main () {
	...
	try {
		/* get CustomLabel summary info */
		const customLabelSummary = await getCustomLabelSummary();
		console.log(customLabelSummary);
		const customLabelsXml = await getCustomLabelXml(['thank_you', 'greeting']);
		console.log(customLabelsXml);
		const customLabels = await parseString(customLabelsXml);
		console.log(JSON.stringify(customLabels, null, 2));
		...
		*/
		...
		/* get field info using metadata api
		const type = {
...
async function getCustomLabelSummary() {
	try {
		return (await conn.metadata.list([{type: 'CustomLabel'}]))
	} catch (err) {
		console.log(err)
	}
}

async function getCustomLabelXml(customLabelFullNames) {
	const writable = fs.createWriteStream('temp.zip');

	const type = {
		members: customLabelFullNames,
		name: 'CustomLabel'
	};
	...
	await conn.metadata.retrieve(params).stream().pipe(writable);
	await finished(writable);
	await decompress('temp.zip', './');
	fs.unlinkSync('temp.zip');
	return fs.readFileSync('./unpackaged/labels/CustomLabels.labels').toString('utf8')
}
...
async function getFieldInfo (objectName) {
	try {
		/* get field info from CustomObject metadata */
		return await conn.metadata.read('CustomObject', [objectName]);
		
		/* get field info using Describe methods
		const objectMetadataDescribe = await conn.describe(objectName);
